                ...
                <table>
                    ...
                    excludedProps = ['version',
                        ...
                        <tr class="prop">
                            ...
                            <% } else if(org.grails.plugins.jdo.JdoUtils.isRelationshipManySide(p)) { 
                                 def referencedClass = org.grails.plugins.jdo.JdoUtils.getReferencedDomainClass(p)
                                 //def dc = grailsApplication.getArtefact("Domain", fullName).propertyName
                                 def dc = new org.codehaus.groovy.grails.commons.DefaultGrailsDomainClass(referencedClass)   %>
                            <td  valign="top" style="text-align:left;" class="value">
                                <ul>
                                <g:each var="${p.name[0]}" in="\${${propertyName}.${p.name}}">
                                    <li><g:link controller="${dc?.propertyName}" action="show" id="\${${p.name[0]}.id}">\${${p.name[0]}?.encodeAsHTML()}</g:link></li>
                                </g:each>
                                </ul>
                            </td>
                            <%  } else if(org.grails.plugins.jdo.JdoUtils.isRelationshipOneSide(p)) { %>
